...
async def get_personal_lessons_schedule(tg_id):
    conn = None
    query = aiosql.from_path("./telegram/db/employee/sql_files/schedule.sql",
...
async def get_webinars_schedule(tg_id):
    conn = None
    query = aiosql.from_path("./telegram/db/employee/sql_files/schedule.sql",
                             driver_adapter="asyncpg")
    ...
    try:
        conn = await asyncpg.connect(config.DB_URI)
        result = await query.get_webinars_schedule(conn, tg_id)
        await conn.close()
        return result
    except (asyncpg.PostgresConnectionError, OSError):
        if conn:
